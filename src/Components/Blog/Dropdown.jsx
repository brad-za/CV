...
const Dropdown = ({ language }) => {
	...
	return (
		<span className="group-one relative z-10 ml-4 mr-20 mt-2 cursor-pointer text-xl font-semibold text-chipWhite md:float-right">
			<NavLink
				className={({ isActive }) =>
			...
			<div className="absolute -right-10">
				...
				>
					{language.categories.map(category => (
						<div
							key={category.slug}
							className="hidden group-one-hover:block"
						>
								...
								<div className="my-2 ml-5 cursor-pointer">
									{category.posts &&
										category.posts.map((post, index) => (
											<div
												key={index}
												className="hidden group-two-hover:flex"
											>
												<NavLink
													className={({ isActive }) =>
														isActive
